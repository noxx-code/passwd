	...
	*,
	...
	def classes_present(s: str) -> set[str]:
		...
		if use_special and any(ch in special for ch in s):
			present.add("special")
		if use_brackets and any(ch in brackets for ch in s):
			present.add("brackets")
		return present
		...
		if seeds:
			...
			while attempts < 1000:
				...
				if require_classes:
					...
					if use_brackets and "brackets" not in present:
						needed.append(brackets)
				...
				if require_classes:
					...
					if use_brackets and "brackets" not in final_present:
						needed_now.append("brackets")
					if needed_now:
